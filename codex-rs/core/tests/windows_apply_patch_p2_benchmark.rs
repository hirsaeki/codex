...
#[test]
fn native_windows_apply_patch_p2_benchmark() -> Result<()> {
...
async fn run_benchmark() -> Result<()> {
    ...
    let target_path = workspace.join("p2-benchmark.txt");
...
async fn run_patch(
    ...
    println!("P2_BENCH_BEGIN {operation}");
    ...
    println!("P2_BENCH_END {operation}");
    Ok(lines.len())
}
