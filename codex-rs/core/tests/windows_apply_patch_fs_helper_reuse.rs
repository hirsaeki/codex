...
static CODEX_DISPATCH: Option<TestBinaryDispatchGuard> = {
    configure_test_binary_dispatch("codex-windows-apply-patch-fs-helper-reuse", |_exe_name, argv1| {
        if matches!(
            argv1,
...
async fn run_test() -> Result<()> {
    ...
    codex_exec_server::with_apply_patch_fs_helper_reuse(codex_apply_patch::apply_patch_with_options(
        "*** Begin Patch\n*** Add File: reuse.txt\n+one\n*** End Patch\n",
