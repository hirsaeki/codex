...
needle = """static SETUP_FLIGHTS: OnceLock<Mutex<HashMap<String, Arc<SetupFlight>>>> = OnceLock::new();

fn run_setup_singleflight(key: String, run: impl FnOnce() -> Result<()>) -> Result<()> {"""
replacement = """static SETUP_FLIGHTS: OnceLock<Mutex<HashMap<String, Arc<SetupFlight>>>> = OnceLock::new();
...
fn run_setup_singleflight(key: String, run: impl FnOnce() -> Result<()>) -> Result<()> {"""
assert text.count(needle) == 1, "singleflight insertion point changed"
...
pub fn sandbox_dir(codex_home: &Path) -> PathBuf {"""
assert text.count(needle) == 1, "cached wrapper insertion point changed"
text = text.replace(needle, replacement)

needle = """    let json = serde_json::to_vec(&payload)?;
...
assert text.count(needle) == 1, "refresh call site changed"
text = text.replace(needle, replacement)

needle = """fn run_setup_exe_payload(
    payload_b64: &str,
    needs_elevation: bool,
    codex_home: &Path,
) -> Result<()> {
    use windows_sys::Win32::System::Threading::GetExitCodeProcess;"""
replacement = """fn run_setup_exe_payload(
    payload_b64: &str,
    needs_elevation: bool,
    codex_home: &Path,
) -> Result<()> {
    ...
    use windows_sys::Win32::System::Threading::GetExitCodeProcess;"""
assert text.count(needle) == 1, "persistent setup invalidation point changed"
text = text.replace(needle, replacement)
...
tests = r'''    #[test]
    ...
    fn concurrent_successful_setup_refreshes_keep_singleflight() {
        ...
        loop {
            ...
            assert!(Instant::now() < deadline, "waiter did not join setup flight");
            thread::yield_now();
        }

        release_tx.send(()).expect("release leader");
        leader.join().expect("leader thread").expect("leader result");
        waiter.join().expect("waiter thread").expect("waiter result");
        assert_eq!(runs.load(Ordering::SeqCst), 1);
    }
    ...
    fn different_setup_refresh_payloads_run_separately() {
        ...
        for key in ["payload-a", "payload-b"] {
            super::run_setup_refresh_cached_with(&cache, key.to_string(), codex_home.path(), || {
                runs.fetch_add(1, Ordering::SeqCst);
                Ok(())
            })
            .expect("refresh succeeds");
    ...
    fn invalidation_forces_setup_refresh_again() {
        ...
        let codex_home = TempDir::new().expect("tempdir");
        let runs = AtomicUsize::new(0);
        super::run_setup_refresh_cached_with(&cache, "invalidate-success".to_string(), codex_home.path(), || {
            runs.fetch_add(1, Ordering::SeqCst);
            Ok(())
        })
    ...
    #[test]
    fn identical_setup_requests_share_one_in_flight_run() {'''
assert text.count(needle) == 1, "test insertion point changed"
text = text.replace(needle, tests)
setup.write_text(text)

identity = Path("codex-rs/windows-sandbox-rs/src/identity.rs")
text = identity.read_text()
needle = """use crate::setup::gather_write_roots_for_permissions;
use crate::setup::offline_proxy_settings_from_env;"""
replacement = """use crate::setup::gather_write_roots_for_permissions;
use crate::setup::invalidate_setup_refresh_success_cache;
use crate::setup::offline_proxy_settings_from_env;"""
assert text.count(needle) == 1, "identity import point changed"
text = text.replace(needle, replacement)

needle = """fn remove_sandbox_users_file(codex_home: &Path, reason: &str) -> Result<()> {
    let path = sandbox_users_path(codex_home);"""
replacement = """fn remove_sandbox_users_file(codex_home: &Path, reason: &str) -> Result<()> {
    ...
    let path = sandbox_users_path(codex_home);"""
assert text.count(needle) == 1, "stale credential invalidation point changed"
text = text.replace(needle, replacement)

needle = """    use crate::setup::SetupMarker;
    use crate::setup::sandbox_users_path;"""
replacement = """    use crate::setup::SetupMarker;
    use crate::setup::sandbox_users_path;
    use crate::setup::setup_refresh_cache_generation_for_tests;"""
assert text.count(needle) == 1, "identity test import point changed"
text = text.replace(needle, replacement)
...
replacement = """    #[test]
    ...
    #[test]
    fn remove_sandbox_users_file_deletes_existing_file() {"""
assert text.count(needle) == 1, "identity test insertion point changed"
text = text.replace(needle, replacement)
