...
- **Public/default**: leave `CODEX_CI_RUNNER_PROFILE` unset (or set it to any
  ...
  uses standard GitHub-hosted runner labels.
- **Upstream/enterprise**: set `CODEX_CI_RUNNER_PROFILE=upstream`. The entrypoint
  ...
  `rust-ci-full.yml`, and `v8-canary.yml` workflows unchanged.
...
| `macos-15-xlarge` | Bazel, Rust CI/full CI, and V8 canary | Replaced by standard arm64 `macos-15` where public cross-platform signal is needed |
...
smaller public runners. Those remain available without modification by opting
into the `upstream` profile.

## Upstream maintenance

Keep the compatibility layer at the two entrypoints (`blocking-ci.yml` and
`postmerge-ci.yml`) plus `public-ci.yml`. Avoid copying changes into the upstream
child workflows unless a public-runner compatibility bug requires it. This
keeps ordinary merges from `openai/codex` focused on a small, stable fork delta.
